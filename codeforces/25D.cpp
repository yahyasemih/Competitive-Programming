...
public:
	IdemSparseTable<T>(int *a, int n = 100010, int k = 25, T (*g)(T, T) = min);
...
int main()
{
	...
	for (auto it = s.begin(); it != s.end(); it++)
	{
		...
		{
			for (int i = 0; i < n; i++)
				if (dsu.find(it->first) != dsu.find(i))
				{
					r.push_back({it->first + 1, it->second + 1, it->first + 1, i + 1});
					dsu.merge(it->first, i);
					break;
				}
				else if (dsu.find(it->second) != dsu.find(i))
				{
					r.push_back({it->first + 1, it->second + 1, it->second + 1, i + 1});
	...
	for (int i = 0; i < r.size(); i++)
		printf("%d %d %d %d\n", get<0>(r[i]), get<1>(r[i]), get<2>(r[i]), get<3>(r[i]));
	return 0;
}
