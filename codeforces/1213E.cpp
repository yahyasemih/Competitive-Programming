...
int main()
{
    ...
    string s; cin >> s;
    string t; cin >> t;
    string possible[] = {"abc", "acb", "bac", "bca", "cab", "cba"};
    vs res;
    for(auto e: possible)
    {
        string ee;
        for(int i = 0; i < n; i++) ee += e;
        res.push_back(ee);
        res.push_back(string(n, e[0]) + string(n,e[1]) + string(n, e[2]));
    }
    for(auto r : res)
    {
        if (r.find(s) == string::npos && r.find(t) == string::npos)
        {
            cout << "YES\n" << r << endl;
            return 0;
        }
