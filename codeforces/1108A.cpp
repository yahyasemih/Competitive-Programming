...
bool correct(vector<ull> t)
{
	for (int i = 0; i < t.size() - 1; i++)
	{
		if (t[i] % 3 == 0)
		{
			if (t[i] != t[i+1] * 3 and t[i] * 2 != t[i+1])
				return false;
		}
		else if (t[i+1] != t[i] * 2)
		{
...
void dfs(vector<int> g[], int n, int u, bool vis[], bool &verdict)
{
	if (vis[u])
	{
		verdict = false;
...
int main()
{
	...
	for (int i = 0; i < q; i++)
	{
		...
		for (int a = l1; a <= r1 and !printed; a++)
		{
			for (int b = l2; b <= r2 and !printed; b++)
			{
				if (a != b)
				{
					cout << a << " " << b << endl;
