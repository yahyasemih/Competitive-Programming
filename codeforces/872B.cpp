...
ll rangeMax(IST *st[], int m, int l, int r)
{
	ll sum = 0;
	for (int i = 0; i < m; i++)
		sum += st[i]->answer(l, r);
...
int main()
{
	...
	for (int i = 0; i < n; i++)
		cin >> a[i];
	IST stMin(a, n, 30);
	IST stMax(a, n, 30, IST::max);
	if (k == 1)
		cout << stMin.answer(0, n-1) << endl;
	else if (k == 2)
	{
		int ans = -2000000000;
		for (int i = 0; i < n - 1; i++)
			ans = max(ans, max(stMin.answer(0, i), stMin.answer(i+1, n-1)));
		cout << ans << endl;
	}
	else
		cout << stMax.answer(0, n-1) << endl;
	return 0;
}
