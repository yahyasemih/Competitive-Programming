...
int main()
{
    ...
    sort(x, x + n);
	vector<ll> alt;
	for (ll p = 1; p <= 2147483648LL; p*=2)
	{
		for (ll i = 0; i < n; i++)
		{
			if (binary_search(x, x + n, x[i] + p) and binary_search(x, x + n, x[i] - p))
			{
				cout << 3 << endl;
				cout << x[i] - p << " " << x[i] << " " << x[i] + p << endl;
				return 0;
			}
			else if (binary_search(x, x + n, x[i] + p) and alt.size() == 0)
			{
				alt.push_back(x[i]);
				alt.push_back(x[i] + p);
			}
